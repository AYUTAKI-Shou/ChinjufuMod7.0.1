...
public class Harbor_Blocks {
	...
	private static CTruss truss() {
		return new CTruss(BlockBehaviour.Properties.of(Material.METAL).strength(1.0F, 6.0F).sound(SoundType.METAL)
				.noOcclusion().isValidSpawn(Harbor_Blocks::neverEntity).isSuffocating(Harbor_Blocks::never));
	}
	
	private static Keiryu keiryu() {
		return new Keiryu(BlockBehaviour.Properties.of(Material.METAL).strength(1.0F, 6.0F).sound(SoundType.METAL)
				.noOcclusion().isValidSpawn(Harbor_Blocks::neverEntity).isSuffocating(Harbor_Blocks::never));
	}
