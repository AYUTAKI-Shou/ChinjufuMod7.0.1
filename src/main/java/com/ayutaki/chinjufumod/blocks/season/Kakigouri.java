...
public class Kakigouri extends BaseFood_Stage4Water {
		...
		if (i != 4) {
			...
			if (stack.isEmpty()) {
				...
				if (i == 1) {
					...
					if (this != Seasonal_Blocks.KAKIGOURI_block) { playerIn.addEffect(new EffectInstance(this.takeEffect(), 600, 0));} }
	
				if (i == 2) {
					if (this == Seasonal_Blocks.KAKIGOURI_block) { playerIn.addEffect(new EffectInstance(Effects.MOVEMENT_SPEED, 500, 0)); }
					if (this != Seasonal_Blocks.KAKIGOURI_block) { playerIn.addEffect(new EffectInstance(this.takeEffect(), 780, 0)); } }
	
				if (i == 3) {
					if (this == Seasonal_Blocks.KAKIGOURI_block) { playerIn.addEffect(new EffectInstance(Effects.MOVEMENT_SPEED, 600, 0)); }
					if (this != Seasonal_Blocks.KAKIGOURI_block) { playerIn.addEffect(new EffectInstance(this.takeEffect(), 900, 0)); } }
	...
	private Effect takeEffect() {
		if (this == Seasonal_Blocks.KAKIGOURI_pink) { return Effects.DAMAGE_BOOST; }
		if (this == Seasonal_Blocks.KAKIGOURI_red) { return Effects.NIGHT_VISION; }
		if (this == Seasonal_Blocks.KAKIGOURI_yellow) { return Effects.DAMAGE_RESISTANCE; }
		if (this == Seasonal_Blocks.KAKIGOURI_green) { return Effects.DIG_SPEED; }
		return null;
	}
