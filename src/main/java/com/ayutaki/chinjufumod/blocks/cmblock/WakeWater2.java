...
public class WakeWater2 extends Base_WakeWater {
	...
	public WakeWater2(BlockBehaviour.Properties properties) {
		super(properties);
	}
	...
	public BlockState updateShape(BlockState state, Direction facing, BlockState facingState, LevelAccessor worldIn, BlockPos pos, BlockPos facingPos) {
		worldIn.scheduleTick(pos, ChinjufuMod_Blocks.WAKE_WATER2.get(), 10);
	...
	public void tick(BlockState state, ServerLevel worldIn, BlockPos pos, RandomSource rand) {
		...
		for(int i = 0; i < 5; ++i) {
			worldIn.scheduleTick(pos, ChinjufuMod_Blocks.WAKE_WATER2.get(), 10);
			worldIn.setBlock(pos, ChinjufuMod_Blocks.WAKE_WATER3.get().defaultBlockState(), 3); //must '3'. 4 does not work. 
		}
	}
	...
	public void animateTick(BlockState state, Level worldIn, BlockPos pos, RandomSource rand) {
		...
		if (rand.nextDouble() < 0.1D) {
			...
			worldIn.playLocalSound(d0, d1, d2, SoundEvents_CM.WATER_WAKE.get(), SoundSource.BLOCKS, 0.5F, 0.7F, false); }
