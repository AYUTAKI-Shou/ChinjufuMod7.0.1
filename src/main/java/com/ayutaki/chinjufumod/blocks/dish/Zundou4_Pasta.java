...
public class Zundou4_Pasta extends BaseZundou_4Cook {
	...
	public Zundou4_Pasta(BlockBehaviour.Properties properties) {
		super(properties);
	}
	...
	public InteractionResult use(BlockState state, Level worldIn, BlockPos pos, Player playerIn, InteractionHand hand, BlockHitResult hit) {
		...
		if (item == Items_Teatime.SARA.get()) {
			if (i == 3) {
				...
				if (stack.isEmpty()) { playerIn.getInventory().add(new ItemStack(Items_Teatime.PASTA_sara.get())); }
				else if (!playerIn.getInventory().add(new ItemStack(Items_Teatime.PASTA_sara.get()))) {
					playerIn.drop(new ItemStack(Items_Teatime.PASTA_sara.get()), false); }
		...
		if (item != Items_Teatime.SARA.get() && item != Items_Teatime.PASTA_nama.get()) { 
