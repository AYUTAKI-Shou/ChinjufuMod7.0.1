...
public class Frypan_OSauce extends BaseFrypan_2Cook {
	...
	public Frypan_OSauce(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 1) {
			if (item == Items.GLASS_BOTTLE) {
				...
				if (stack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.OSAUCE_bot_1, 1)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.OSAUCE_bot_1, 1))) {
					playerIn.drop(new ItemStack(Items_Teatime.OSAUCE_bot_1, 1), false); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add(new TranslationTextComponent("tips.block_food_frypan_osauce").withStyle(TextFormatting.GRAY));
	}
}
