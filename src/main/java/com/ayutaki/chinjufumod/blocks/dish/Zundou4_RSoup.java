...
public class Zundou4_RSoup extends BaseStage4_Face {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		boolean tare = (item == Items_Teatime.RAMEN_nama && (k == 1 || k == 2 || k == 3));
		
		if (tare) {
			/** Collect with an Item **/
			CMEvents.Consume1Item_Bowl(worldIn, pos, playerIn, hand);

			if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.RAMEN_nama, 1, k + 3)); }
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.RAMEN_nama, 1, k + 3))) {
				playerIn.dropItem(new ItemStack(Items_Teatime.RAMEN_nama, 1, k + 3), false); }
		...
		if (item != Items_Teatime.RAMEN_nama || k == 0) { CMEvents.textNotHave(worldIn, pos, playerIn); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		if (i == 1) { stack.add(new ItemStack(Items_Teatime.ZUNDOU_RSOUP, 1, 1)); }
