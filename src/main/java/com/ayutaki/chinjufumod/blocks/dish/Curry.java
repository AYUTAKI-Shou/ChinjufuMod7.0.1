...
public class Curry extends BaseFood_Stage4Water {
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 4) {
			...
			if (stack.isEmpty()) {
				...
				if (this == Dish_Blocks.CURRY_T) {
					if (i == 1) {
						/** -600 **/
						playerIn.addEffect(new EffectInstance(Effects.DIG_SPEED, 3300, 0));
					...
					if (i == 3) {
						playerIn.addEffect(new EffectInstance(Effects.HEAL, 0, 0));
						playerIn.addEffect(new EffectInstance(Effects.SATURATION, 4, 0));
						playerIn.addEffect(new EffectInstance(Effects.REGENERATION, 3300, 0)); }
				}
				
				if (this != Dish_Blocks.CURRY_T) {
					if (i == 1) {
						/** 採掘3600 1秒＝20 満腹は2で肉メモリの1個分 +300 **/
						playerIn.addEffect(new EffectInstance(Effects.DIG_SPEED, 3900, 0));
						playerIn.addEffect(new EffectInstance(Effects.SATURATION, 4, 0)); }
	...
	private ItemStack takeStack() {
		if (this == Dish_Blocks.CURRY) { return new ItemStack(Items_Teatime.CURRY, 1); }
		if (this == Dish_Blocks.CURRY_C) { return new ItemStack(Items_Teatime.CURRY_C, 1); }
		if (this == Dish_Blocks.CURRY_T) { return new ItemStack(Items_Teatime.CURRY_T, 1); }
		return null;
	}
}
