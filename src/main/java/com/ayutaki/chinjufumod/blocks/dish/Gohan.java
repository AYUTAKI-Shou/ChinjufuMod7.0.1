...
public class Gohan extends BaseStage3_FaceDown {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (i != 3) {
			...
			if (stack.isEmpty()) {
				...
				if (this != Dish_Blocks.GOHAN) {
					if (i == 1) {
						((EntityLivingBase) playerIn).addPotionEffect(new PotionEffect(MobEffects.HASTE, 100, 0));
	...
	private ItemStack takeStack() {
		if (this == Dish_Blocks.GOHAN) { return new ItemStack(Items_Teatime.GOHAN, 1, 0); }
		if (this == Dish_Blocks.GOHAN_KURI) { return new ItemStack(Items_Teatime.GOHAN_KURI, 1, 0); }
		if (this == Dish_Blocks.GOHAN_TAKE) { return new ItemStack(Items_Teatime.GOHAN_TAKE, 1, 0); }
		return null;
	}
}
