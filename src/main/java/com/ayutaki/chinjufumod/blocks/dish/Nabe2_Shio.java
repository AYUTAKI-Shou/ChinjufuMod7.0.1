...
public class Nabe2_Shio extends BaseNabe_2Cook {
	...
	public Nabe2_Shio(BlockBehaviour.Properties properties) {
		super(properties);
	}
	...
	public InteractionResult use(BlockState state, Level worldIn, BlockPos pos, Player playerIn, InteractionHand hand, BlockHitResult hit) {
		...
		if (i != 1) {
			...
			if (stack.isEmpty()) {
				...
				if (!playerIn.getInventory().add(new ItemStack(Items_Teatime.NIGARI.get(), 1))) { playerIn.drop(new ItemStack(Items_Teatime.NIGARI.get(), 1), false); }
	...
	public void appendHoverText(ItemStack stack, @Nullable BlockGetter worldIn, List<Component> tooltip, TooltipFlag tipFlag) {
		tooltip.add(new TranslatableComponent("tips.block_nabe").withStyle(ChatFormatting.GRAY));
		tooltip.add(new TranslatableComponent("tips.block_food_nabetoufu_b").withStyle(ChatFormatting.GRAY));
	}
}
