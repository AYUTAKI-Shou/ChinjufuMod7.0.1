	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (kirimi) {
			if (item == Items_Teatime.KIRIMI_S) { 
				...
				if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_S, 1)); }
				else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_S, 1))) {
					playerIn.dropItem(new ItemStack(Items_Teatime.SUSHI_S, 1), false); } }
			
			if (item == Items_Teatime.KIRIMI_F) { 
				CMEvents.Consume1Item_SnowB(worldIn, pos, playerIn, hand);
				
				if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_F, 1)); }
				else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_F, 1))) {
					playerIn.dropItem(new ItemStack(Items_Teatime.SUSHI_F, 1), false); } }
			
			if (item == Items_Teatime.KIRIMI_B) { 
				CMEvents.Consume1Item_SnowB(worldIn, pos, playerIn, hand);
				
				if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_B, 1)); }
				else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_B, 1))) {
					playerIn.dropItem(new ItemStack(Items_Teatime.SUSHI_B, 1), false); } }
			
			if (item == Items_Teatime.KIRIMI_T) { 
				CMEvents.Consume1Item_SnowB(worldIn, pos, playerIn, hand);
				
				if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_T, 1)); }
				else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.SUSHI_T, 1))) {
					playerIn.dropItem(new ItemStack(Items_Teatime.SUSHI_T, 1), false); } }
			...
			if (i == 15) {
				worldIn.setBlockState(pos, Dish_Blocks.SUSHIOKE_FULL_1.getDefaultState()
						.with(BaseFood_Stage9Water.H_FACING, Direction.NORTH)
						.with(BaseFood_Stage9Water.DOWN, state.get(DOWN))
						.with(BaseFood_Stage9Water.STAGE_1_9, Integer.valueOf(9)), 3); }
		}
