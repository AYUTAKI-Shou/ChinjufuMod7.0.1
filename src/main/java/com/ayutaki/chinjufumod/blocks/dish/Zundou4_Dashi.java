...
public class Zundou4_Dashi extends BaseZundou_4Stage {
	...
	public Zundou4_Dashi(BlockBehaviour.Properties properties) {
		super(properties);
	}
	...
	public InteractionResult use(BlockState state, Level worldIn, BlockPos pos, Player playerIn, InteractionHand hand, BlockHitResult hit) {
		...
		if (item == Items_Teatime.SHOUYU_donburi.get()) {
			/** Collect with an Item **/
			CMEvents.Consume1Item_Bowl(worldIn, pos, playerIn, hand);

			if (stack.isEmpty()) { playerIn.getInventory().add(new ItemStack(Items_Teatime.TSUYU_donburi.get())); }
			else if (!playerIn.getInventory().add(new ItemStack(Items_Teatime.TSUYU_donburi.get()))) {
				playerIn.drop(new ItemStack(Items_Teatime.TSUYU_donburi.get()), false); }
		...
		if (item != Items_Teatime.SHOUYU_donburi.get() && item != Items_Teatime.DASHI_bot_1.get()) { 
	...
	public ItemStack getCloneItemStack(BlockGetter worldIn, BlockPos pos, BlockState state) {
		return new ItemStack(Items_Teatime.ZUNDOU_DASHI.get());
	}
	
	/* ToolTip */
	public void appendHoverText(ItemStack stack, @Nullable BlockGetter worldIn, List<Component> tooltip, TooltipFlag tipFlag) {
		tooltip.add(new TranslatableComponent("tips.block_food_dashinabe_1").withStyle(ChatFormatting.GRAY));
	}
}
