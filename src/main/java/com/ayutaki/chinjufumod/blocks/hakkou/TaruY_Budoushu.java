	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i > 3) {
			if (item == Items_Teatime.SAKEBOTTLE) {
				...
				if (i == 4 || i == 5) {
					if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.WINEBOT)); }
				...
				if (i == 6 && item == Items_Teatime.SAKEBOTTLE) {
					if (stack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.JUKUWINEBOT)); }
