...
public class ReizouTop extends AbstractReizouTop<ReizouTop_TileEntity> {
	...
	/* Collision */
	protected static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 10.0D, 13.4D);
	protected static final VoxelShape AABB_WEST = Block.makeCuboidShape(2.6D, 0.0D, 0.0D, 16.0D, 10.0D, 16.0D);
	protected static final VoxelShape AABB_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 2.6D, 16.0D, 10.0D, 16.0D);
	protected static final VoxelShape AABB_EAST = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 13.4D, 10.0D, 16.0D);
	
	public ReizouTop(Block.Properties properties, Supplier<TileEntityType<? extends ReizouTop_TileEntity>> entityType) {
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (state.getBlock() != newState.getBlock()) {
			TileEntity tileentity = worldIn.getTileEntity(pos);
			if (tileentity instanceof ReizouTop_TileEntity) {
				InventoryHelper.dropItems(worldIn, pos, ((ReizouTop_TileEntity) tileentity).getItems());
			}
		}
	}
