...
public class TobukuroWindowL extends BaseStage3_Face {
		...
		if (i != 3) {
			...
			default :
				if (eastState.getMaterial().isReplaceable()) {
					...
					worldIn.setBlockState(pos.east(), win_FACE.withProperty(BaseStage2_Face.STAGE_1_2, Integer.valueOf(2))); }
			...
			case SOUTH :
				if (westState.getMaterial().isReplaceable()) {
					CMEvents.soundFusumaS(worldIn, pos);
					worldIn.setBlockState(pos, state.withProperty(STAGE_1_3, Integer.valueOf(i + 1)));
					worldIn.setBlockState(pos.west(), win_FACE.withProperty(BaseStage2_Face.STAGE_1_2, Integer.valueOf(2))); }
			...
			case EAST :
				if (southState.getMaterial().isReplaceable()) {
					CMEvents.soundFusumaS(worldIn, pos);
					worldIn.setBlockState(pos, state.withProperty(STAGE_1_3, Integer.valueOf(i + 1)));
					worldIn.setBlockState(pos.south(), win_FACE.withProperty(BaseStage2_Face.STAGE_1_2, Integer.valueOf(2))); }
			...
			case WEST :
				if (northState.getMaterial().isReplaceable()) {
					CMEvents.soundFusumaS(worldIn, pos);
					worldIn.setBlockState(pos, state.withProperty(STAGE_1_3, Integer.valueOf(i + 1)));
					worldIn.setBlockState(pos.north(), win_FACE.withProperty(BaseStage2_Face.STAGE_1_2, Integer.valueOf(2))); }
	...
	private Block takeBlock() {
		...
		if (this == Slidedoor_Blocks.TOBUKUROS_WINL) { return Slidedoor_Blocks.AMADOS_WIN; }
		return null;
	}
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		if (this == Slidedoor_Blocks.TOBUKURO_WINL) { stack.add(new ItemStack(Items_Wadeco.TOBUKURO_WINR)); }
		if (this == Slidedoor_Blocks.TOBUKUROS_WINL) { stack.add(new ItemStack(Items_Wadeco.TOBUKUROS_WINR)); }
		return stack;
	}

	@Override
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World worldIn, BlockPos pos, EntityPlayer playerIn) {
		if (this == Slidedoor_Blocks.TOBUKUROS_WINL) { return new ItemStack(Items_Wadeco.TOBUKUROS_WINR); }
