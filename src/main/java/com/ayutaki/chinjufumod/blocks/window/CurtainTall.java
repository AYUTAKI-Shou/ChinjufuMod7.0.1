...
public class CurtainTall extends CM_WaterLogged {
	...
	protected static final VoxelShape OPENR_NORTH = Block.makeCuboidShape(14.0D, 0.0D, 0.0D, 16.0D, 16.0D, 2.1D);
	protected static final VoxelShape OPENR_EAST = Block.makeCuboidShape(13.9D, 0.0D, 14.0D, 16.0D, 16.0D, 16.0D);
	protected static final VoxelShape OPENR_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 13.9D, 2.0D, 16.0D, 16.0D);
	protected static final VoxelShape OPENR_WEST = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 2.1D, 16.0D, 2.0D);

	protected static final VoxelShape OPENL_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 2.0D, 16.0D, 2.1D);
	protected static final VoxelShape OPENL_EAST = Block.makeCuboidShape(13.9D, 0.0D, 0.0D, 16.0D, 16.0D, 2.0D);
	protected static final VoxelShape OPENL_SOUTH = Block.makeCuboidShape(14.0D, 0.0D, 13.9D, 16.0D, 16.0D, 16.0D);
	protected static final VoxelShape OPENL_WEST = Block.makeCuboidShape(0.0D, 0.0D, 14.0D, 2.1D, 16.0D, 16.0D);
		...
		if (worldIn.getBlockState(pos.down()).isReplaceable(context)) {
			...
			if (playerIn.isSneaking()) {
				return this.getDefaultState().with(H_FACING, context.getPlacementHorizontalFacing()).with(WATERLOGGED, fluid.getFluid() == Fluids.WATER)
						.with(HINGE, DoorHingeSide.RIGHT).with(OPEN, Boolean.valueOf(false)).with(HALF, DoubleBlockHalf.UPPER); }

			return this.getDefaultState().with(H_FACING, context.getPlacementHorizontalFacing()).with(WATERLOGGED, fluid.getFluid() == Fluids.WATER)
					.with(HINGE, DoorHingeSide.LEFT).with(OPEN, Boolean.valueOf(false)).with(HALF, DoubleBlockHalf.UPPER);
		}
