...
public class RenderAmmo_Small<T extends AbstractAmmo_Entity> extends EntityRenderer<T> {
	...
	public void render(T entityIn, float entityYaw, float partialTicks, PoseStack stackIn, MultiBufferSource bufferIn, int packedLightIn) {
		stackIn.pushPose();
		stackIn.mulPose(Axis.YP.rotationDegrees(Mth.lerp(partialTicks, entityIn.yRotO, entityIn.getYRot()) - 90.0F)); //1.18->1.20
		stackIn.mulPose(Axis.ZP.rotationDegrees(Mth.lerp(partialTicks, entityIn.xRotO, entityIn.getXRot()))); //1.18->1.20
		int i = 0;
		float f = 0.0F;
		float f1 = 0.5F;
		float f2 = 0.0F;
		...
		if (f9 > 0.0F) {
			...
			stackIn.mulPose(Axis.ZP.rotationDegrees(f10)); //1.18->1.20
		}

		stackIn.mulPose(Axis.XP.rotationDegrees(45.0F)); //1.18->1.20
		...
		for(int j = 0; j < 4; ++j) {
			stackIn.mulPose(Axis.XP.rotationDegrees(90.0F)); //1.18->1.20
