...
public class RensouHou356S3 extends Base_RensouHou {
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.capabilities.isCreativeMode || (projectile.getItem() instanceof Ammo_Large && ((Ammo_Large) projectile.getItem()).isInfinite(projectile, stack, playerIn));

					if (!worldIn.isRemote) {
						/* Ammo item instance. Entity to be fired. */
						Ammo_Large itemarrow = (Ammo_Large)(projectile.getItem() instanceof Ammo_Large ? projectile.getItem() : Items_Weapon.AMMUNITION_L);
						AmmoEntity_Large entityarrow = (AmmoEntity_Large) itemarrow.createAmmo(worldIn, projectile, playerIn);
						...
						double FIT = (battleship)? 2.0D : 0.0D;
						...
						entityarrow.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, (battleship)? 4.0F : 3.0F, 1.0F);
						if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 6.0D + FIT + LEVEL); }
						if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + 6.0D + (double)j * 0.5D + FIT + LEVEL); }
	...
	protected boolean isArrow(ItemStack stack) {
		return stack.getItem() instanceof Ammo_Large;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("tips.item_rensouhou_large.name", meta));
		tooltip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_rensouhou356_s3.name", meta));
	}
}
