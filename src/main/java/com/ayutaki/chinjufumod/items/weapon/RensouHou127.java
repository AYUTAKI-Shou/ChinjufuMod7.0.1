...
public class RensouHou127 extends Base_RensouHou {
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.capabilities.isCreativeMode || (projectile.getItem() instanceof Ammo_Small && ((Ammo_Small) projectile.getItem()).isInfinite(projectile, stack, playerIn));

					if (!worldIn.isRemote) {
						/* Ammo item instance. Entity to be fired. */
						Ammo_Small itemarrow = (Ammo_Small)(projectile.getItem() instanceof Ammo_Small ? projectile.getItem() : Items_Weapon.AMMUNITION_S);
						AmmoEntity_Small entityarrow = (AmmoEntity_Small) itemarrow.createAmmo(worldIn, projectile, playerIn);
						...
						if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + FIT + LEVEL); }
						if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + (double)j * 0.5D + FIT + LEVEL); }
	...
	protected boolean isArrow(ItemStack stack) {
		return stack.getItem() instanceof Ammo_Small;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("tips.item_rensouhou_small.name", meta));
		tooltip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_rensouhou127.name", meta));
	}
}
