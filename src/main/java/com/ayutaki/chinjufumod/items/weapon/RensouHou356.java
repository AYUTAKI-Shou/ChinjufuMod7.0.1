...
public class RensouHou356 extends Base_RensouHou {
	...
	public RensouHou356(Item.Properties properties) {
		super(properties);
	}
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						...
						if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.5D + FIT + LEVEL); }
						if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.5D + (double)j * 0.5D + FIT + LEVEL); }
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag tipFlag) {
		...
		tooltip.add(new TranslatableComponent("tips.item_rensouhou356").withStyle(ChatFormatting.DARK_GREEN));
	}
}
