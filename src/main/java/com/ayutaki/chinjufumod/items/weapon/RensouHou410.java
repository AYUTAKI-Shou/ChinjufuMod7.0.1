...
public class RensouHou410 extends Base_RensouHou {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						...
						if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 6.0D + FIT + LEVEL); }
						if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 6.0D + (double)j * 0.5D + FIT + LEVEL); }
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add(new TranslationTextComponent("tips.item_rensouhou410").applyTextStyle(TextFormatting.DARK_GREEN));
	}
}
