...
public class RensouHou410 extends Base_RensouHou {
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.abilities.instabuild || (projectile.getItem() instanceof Ammo_Large && ((Ammo_Large)projectile.getItem()).isInfinite(projectile, stack, playerIn));
					...
					if (!worldIn.isClientSide) {
						...
						abstractarrowentity.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, (battleship)? 4.0F : 3.0F, 1.0F);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add(new TranslationTextComponent("tips.item_rensouhou410").withStyle(TextFormatting.DARK_GREEN));
	}
}
