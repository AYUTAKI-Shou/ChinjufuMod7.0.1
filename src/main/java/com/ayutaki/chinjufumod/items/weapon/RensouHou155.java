			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						...
						if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 1.5D + FIT + LEVEL); }
						if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 1.5D + (double)j * 0.5D + FIT + LEVEL); }
