...
public class RensouHou380 extends Base_RensouHou {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						...
						if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 5.0D + FIT + LEVEL); }
						if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 5.0D + (double)j * 0.5D + FIT + LEVEL); }
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag tipFlag) {
		...
		tooltip.add(new TranslatableComponent("tips.item_rensouhou380").withStyle(ChatFormatting.DARK_GREEN));
	}
}
