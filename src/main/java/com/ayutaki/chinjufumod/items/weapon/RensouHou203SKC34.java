...
public class RensouHou203SKC34 extends Base_RensouHou {

	public RensouHou203SKC34(Item.Properties properties) {
		super(properties);
	}
		...
		if (entityLiving instanceof Player) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.getAbilities().instabuild || (projectile.getItem() instanceof Ammo_Medium && ((Ammo_Medium)projectile.getItem()).isInfinite(projectile, stack, playerIn));
					...
					if (!worldIn.isClientSide) {
						...
						abstractarrowentity.shootFromRotation(playerIn, playerIn.getXRot(), playerIn.getYRot(), 0.0F, 3.5F, 1.0F);
						if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 3.0D + FIT + LEVEL); }
						if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 3.0D + (double)j * 0.5D + FIT + LEVEL); }
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag tipFlag) {
		tooltip.add(new TranslatableComponent("tips.item_rensouhou_medium").withStyle(ChatFormatting.GRAY));
		tooltip.add(new TranslatableComponent("tips.item_rensouhou203_skc34").withStyle(ChatFormatting.DARK_GREEN));
	}
}
