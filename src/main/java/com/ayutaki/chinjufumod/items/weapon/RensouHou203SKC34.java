		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.abilities.isCreativeMode || (projectile.getItem() instanceof Ammo_Medium && ((Ammo_Medium)projectile.getItem()).isInfinite(projectile, stack, playerIn));
					...
					if (!worldIn.isRemote) {
						...
						abstractarrowentity.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, 3.5F, 1.0F);
						if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 3.0D + FIT + LEVEL); }
						if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 3.0D + (double)j * 0.5D + FIT + LEVEL); }
