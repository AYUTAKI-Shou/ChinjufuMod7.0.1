...
public class RensouHou203SKC34 extends Base_RensouHou {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						...
						if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + FIT + LEVEL); }
						if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + (double)j * 0.5D + FIT + LEVEL); }
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_rensouhou203_skc34.name", meta));
	}
}
