...
public class BPlate_Item extends Item {

	public BPlate_Item(String name) {
		super();
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!block.isReplaceable(worldIn, pos)) { pos = pos.offset(facing); }
			...
			if (!stack.isEmpty() && playerIn.canPlayerEdit(pos, facing, stack) && worldIn.mayPlace(this.takeBlock(playerIn, hand), pos, false, facing, (Entity)null)) {
				...
				IBlockState state1 = this.takeBlock(playerIn, hand).getDefaultState().withProperty(BambooPressurePlate.H_FACING, direction);
	...
	private Block takeBlock(EntityPlayer playerIn, EnumHand hand) {
		...
		if (k == 2) { return JPDeco_Blocks.TAKE_PLATE_K; }
		return null;
	}
}
