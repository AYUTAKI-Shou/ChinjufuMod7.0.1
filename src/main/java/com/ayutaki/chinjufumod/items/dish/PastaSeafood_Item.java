...
public class PastaSeafood_Item extends ItemFood {

	public PastaSeafood_Item(String name, int amount, float saturation, boolean isWolfFood) {
	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase entityLiving) {
		...
		if (playerIn == null || !playerIn.capabilities.isCreativeMode) {
			...
			if (stack.isEmpty()) { return new ItemStack(Items_Teatime.HAMAGURI_KARA, 3, 0); }
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.HAMAGURI_KARA, 3, 0))) {
				playerIn.dropItem(new ItemStack(Items_Teatime.HAMAGURI_KARA, 3, 0), false); }
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!stack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, stack) && worldIn
				.mayPlace(Dish_Blocks.PASTASEAFOOD, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

			/** Put the Block. **/
			IBlockState state1 = Dish_Blocks.PASTASEAFOOD.getDefaultState()
