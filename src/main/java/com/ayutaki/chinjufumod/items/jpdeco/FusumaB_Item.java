...
public class FusumaB_Item extends Item {

	public FusumaB_Item(String name) {
		super();
	...
	public static void placeDoor_right(World worldIn, BlockPos pos, EnumFacing facing, Block door) {
		...
		IBlockState state = door.getDefaultState().withProperty(Fusuma.H_FACING, facing)
				.withProperty(Fusuma.HINGE, HingeState.RIGHT)
				.withProperty(Fusuma.POWERED, Boolean.valueOf(flag2))
				.withProperty(Fusuma.OPEN, Boolean.valueOf(flag2));
		worldIn.setBlockState(pos, state.withProperty(Fusuma.HALF, HalfState.LOWER), 2);
		worldIn.setBlockState(upPos, state.withProperty(Fusuma.HALF, HalfState.UPPER), 2);
	...
	public static void placeDoor_left(World worldIn, BlockPos pos, EnumFacing facing, Block door) {
		...
		boolean flag2 = worldIn.isBlockPowered(pos) || worldIn.isBlockPowered(upPos);
		IBlockState state = door.getDefaultState().withProperty(Fusuma.H_FACING, facing)
				.withProperty(Fusuma.HINGE, HingeState.LEFT)
				.withProperty(Fusuma.POWERED, Boolean.valueOf(flag2))
				.withProperty(Fusuma.OPEN, Boolean.valueOf(flag2));
		worldIn.setBlockState(pos, state.withProperty(Fusuma.HALF, HalfState.LOWER), 2);
		worldIn.setBlockState(upPos, state.withProperty(Fusuma.HALF, HalfState.UPPER), 2);
	...
	private Block takeBlock(EntityPlayer playerIn, EnumHand hand) {
		...
		if (k == 15) { return Fusuma_Blocks.FUSUMAB_black; }
		return null;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("tips.block_fusuma.name", meta));
	}
}
